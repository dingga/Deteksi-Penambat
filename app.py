...
try:
    ...
    st.info("Pastikan requirements.txt HANYA berisi 'opencv-python-headless'.")
    st.stop()
...
if model is None:
    st.error("File 'best.pt' tidak ditemukan di repositori.")
    st.stop()
...
if uploaded_video:
    ...
    # ROI & Garis Hitung (Sesuai parameter tesis Anda)
    y_ref = int(0.6 * h)
                    ...
                    if cv2.pointPolygonTest(roi, (cx, cy), False) >= 0:
                        ...
                        if cy > y_ref and tid not in counted:
                            ...
                            if "Hilang" in final_lbl:
                                ...
                                # Update Galeri secara real-time (4 terbaru)
