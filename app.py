...
try:
    ...
    st.info("Pastikan file 'requirements.txt' sudah menggunakan 'opencv-python-headless'.")
    st.stop()
...
st.markdown("Aplikasi ini mendeteksi komponen penambat (E-Clip, DE-Clip, KA-Clip) dan mengambil **Screenshot** otomatis jika terdeteksi **Hilang**.")
...
# Memastikan path model absolut agar terbaca di Cloud
...
if model is None:
    ...
    st.sidebar.success("✅ Model Berhasil Dimuat (5.6 MB)")
...
if uploaded_video is not None:
    ...
    # PERBAIKAN: Menggunakan konstanta yang benar untuk OpenCV
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    
    # ROI Melayang (Floating ROI) sesuai parameter tesis
    ...
    y_ref = int(0.6 * height) # Garis hitung biru
    ...
    if st.sidebar.button("Mulai Analisis"):
        # Bersihkan folder screenshot lama
        ...
        for frame_idx, res in enumerate(results):
            ...
            if res.boxes is not None and res.boxes.id is not None:
                ...
                for box, tid, cls in zip(boxes, ids, clss):
                    ...
                    if cv2.pointPolygonTest(roi_points, (cx, cy), False) >= 0:
                        ...
                        # Logika Hitung & Simpan Screenshot
            ...
            with stats_placeholder.container():
                ...
                st.table(pd.DataFrame({"Jenis": summary_counts.keys(), "Unit": summary_counts.values()}))
                if len(captured_files) > 0:
                    st.warning(f"📸 {len(captured_files)} Foto Bukti Hilang Tersimpan")

        # Fitur Download ZIP Hasil Screenshot
        if captured_files:
            zip_path = "dokumentasi_tesis.zip"
